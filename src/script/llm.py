...
7. Для строк прямой речи ОБЯЗАТЕЛЬНО добавляй префикс спикера перед каждой
   репликой:
     Рассказчик: ...обычная нарративная строка.
     Герой: — А ты что, не знал?
     Героиня: — Знала. Но не думала, что это так закончится.
   Правила:
     - префикс идёт ТОЛЬКО в начале логической строки (после \n).
     - для нарративного текста префикс "Рассказчик:" ставь перед каждым абзацем.
     - имена спикеров — короткие (до 20 символов), Первая Буква Заглавная.
     - если у персонажа нет имени, используй "Герой" / "Героиня".
   Префиксы позволяют downstream TTS использовать многоголосую озвучку для
   диалогов — не убирай их.
"""
